...
def calculate_interest(input_file, output_file, conversion_file, summary_file):
    ...
    # Calculate NTD
    df['ntd'] = df.apply(lambda row: int(row['price'] * conversion_rate) if row['price'] != 'Invalid Data' else 'Invalid Data', axis=1)
    ...
    with open(summary_file, 'a', newline='') as summary:
        writer = pd.DataFrame([['cash_us', total_ntd]], columns=['name', 'value'])
